...
class Auth extends CI_Controller
{
	...
	public function register(){
        ...
        {
            $this->form_validation->set_rules('photo', $this->lang->line('register_photo_label'), 'trim');
        }
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if (!file_exists($_FILES['photo']['tmp_name']) || !is_uploaded_file($_FILES['photo']['tmp_name'])){
                ...
                $typ = explode('.',$_FILES["photo"]["name"]);
                $typ = $typ[count($typ) - 1];
                $namePhoto = trim(url_create($this->input->post('first_name')));
                $photo = "photo/".$namePhoto.'-'.uniqid().'.'.$typ;
